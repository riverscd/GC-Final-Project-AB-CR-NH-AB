...
communitiesRoutes.get("/communities/bylocation/:location", (req, res) => {
  ...
  return db
    .manyOrNone("select * from communities where location = $(location)", {
...
communitiesRoutes.get("/communities/bycreator/:creator_id", (req, res) => {
  return db
    .manyOrNone("select * from communities where creator_id = $(creator_id)", {
...
communitiesRoutes.post(
  ...
  (req: any, res: any) => {
    return db
      .manyOrNone("SELECT * FROM communities WHERE id IN ($1:csv);", [
        req.body.added_community_ids,
      ])
...
communitiesRoutes.post("/create-community", (req, res) => {
  const communitySchema = Joi.object({
    community_name: Joi.string().min(2).max(50).required(),
